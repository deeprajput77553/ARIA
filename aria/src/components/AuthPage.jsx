...
const AuthPage = ({ mode = 'signin', onSwitch }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Integration placeholder — wire to your auth backend
      ...
      <div className="auth-card">
        ...
        <div className="auth-logo">
          <svg viewBox="0 0 40 40" width="44" height="44">
            ...
            <circle cx="20" cy="20" r="18" fill="none" stroke="url(#al)" strokeWidth="1.5" opacity="0.7"/>
            <circle cx="20" cy="20" r="12" fill="none" stroke="url(#al)" strokeWidth="1"   opacity="0.4"/>
            <circle cx="20" cy="20" r="4"  fill="url(#al)"/>
            {[0,60,120,180,240,300].map((deg,i) => {
              const r = deg * Math.PI / 180;
              return <circle key={i} cx={20 + Math.cos(r)*12} cy={20 + Math.sin(r)*12} r="1.8" fill="url(#al)" opacity="0.7"/>;
            })}
          </svg>
          <span>ARIA</span>
        </div>
        ...
        <p className="auth-sub">{isSignIn ? 'Sign in to your ARIA workspace' : 'Start your AI-powered journey'}</p>
