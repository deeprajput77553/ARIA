...
const Chat = ({ onNavigate }) => {
  ...
  const [isMuted, setIsMuted] = useState(!settings.voiceEnabled);
  ...
  useEffect(() => {
    ...
    const handleKey = (e) => { if (e.key === 'Escape') stopSpeaking(); };
    ...
    <div className="chat-fullscreen">
      <header className="cf-header">
        ...
        <div className="cf-header-right">
          ...
          <button 
            ...
            onClick={async () => {
              const res = await fetch('http://localhost:3001/git/snapshot', {
                ...
                body: JSON.stringify({ message: 'Manual save from Chat' })
              });
              const data = await res.json();
              if (data.ok) alert('Successfully saved to Git: ' + data.message);
          ...
          <button 
            className={`cf-icon-btn cf-mute ${isMuted ? 'active' : ''}`} 
            onClick={() => {
              const newMute = !isMuted;
              setIsMuted(newMute);
              const s = loadSettings();
              s.voiceEnabled = !newMute;
              localStorage.setItem('aria_settings', JSON.stringify(s));
          ...
          <div key={m.id} className={`cf-wrap ${m.role === 'user' ? 'cf-user-wrap' : 'cf-ai-wrap'}`}>
            ...
            <div className="cf-bubble">
              <div className="cf-meta">
                <div className={`cf-status-pill ${m.role}`}>
                  <span className="pill-dot" />
                  {m.role === 'user' ? 'Operator' : 'ARIA Core'}
                </div>
                <span className="cf-time">{String(m.time ?? '')}</span>
                ...
                <div className="cf-steps">
                  {m.steps.map((s, i) => (
                    <div key={i} className={`cf-step ${s.status}`}>
                      ...
                      <span>{typeof s.label === 'string' ? s.label : JSON.stringify(s.label)}</span>
                    </div>
                  ))}
                </div>
              )}
